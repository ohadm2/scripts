...
# ─── detect the damaged root ──────────────────────────────────────────────────
detect_root() {
  [[ -n "$ROOT_DEV" ]] && { echo "$ROOT_DEV"; return 0; }
  local parts=() dev fstype
  while IFS= read -r dev fstype; do
    [[ -b "$dev" ]] || continue
    is_live_medium "$dev" && continue
    parts+=("$dev|$fstype")
  done < <(lsblk -lnpo NAME,FSTYPE | awk '$2 ~ /^(ext[234]|xfs|btrfs)$/')

  (( ${#parts[@]} )) || die "No non-live Linux partitions found. Use --root /dev/XXX."
  if (( ${#parts[@]} == 1 )); then echo "${parts[0]%%|*}"; return 0; fi
  warn "Multiple Linux partitions found — which one was the damaged system?"
  local i
  for i in "${!parts[@]}"; do
    local d="${parts[$i]%%|*}" f="${parts[$i]##*|}" sz
    sz=$(lsblk -dno SIZE "$d" 2>/dev/null | tr -d '[:space:]')
    printf "   [%d] %-18s %-6s %s\n" "$((i+1))" "$d" "$f" "$sz" >&2
  done
  local sel; read -rp "Select [1-${#parts[@]}]: " sel
  [[ "$sel" =~ ^[0-9]+$ ]] && (( sel>=1 && sel<=${#parts[@]} )) || die "Invalid selection."
  echo "${parts[$((sel-1))]%%|*}"
}
